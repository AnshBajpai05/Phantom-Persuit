...
def connect_to_db():
    ...
    return mysql.connector.connect(
        ...
        port=8080,
        database="ghost_game"  # Replace with your database name
    )
...
def visualize_game_state(game):
    if not hasattr(game, 'G'):
        ...
        for u, v in edges:
            # Example: weight based on Euclidean distance between nodes
            x1, y1 = game.pos[u]
            x2, y2 = game.pos[v]
            distance = ((x2 - x1)**2 + (y2 - y1)**2)**0.5
            game.G[u][v]['weight'] = round(distance, 2)  # Set the edge weight (rounded for readability)
    ...
    # Add heuristic (edge weights) for difficulty level 3
    if game.difficulty == 3:
        edge_labels = nx.get_edge_attributes(game.G, 'weight')  # Get the edge weights
...
class Game:
    ...
    def move_ghost(self):
        status_text1 = plt.text(0.5, 1.00, "", transform=plt.gca().transAxes, ha="center", fontsize=12, color="green")
    ...
    def handle_ghost_encounter(self):
        ...
        while self.hearts_of_dead > 0:
            ...
            if respawn_choice == 'y':
                ...
                # Reset ghost and player positions
                self.player_position = random.randint(1, 25)  # Reset player position
                self.ghost_position = self.get_distant_ghost_position()  # Ensure ghost is distant
    ...
    def collect_powerup(self):
        status_text = plt.text(0.5, 1.15, "", transform=plt.gca().transAxes, ha="center", fontsize=12, color="blue")
    ...
    def display_loading_screen(self):
        ...
        image = Image.open("Sound/loading.png")  # Replace with your image path
        ...
        # Schedule the closure of the loading screen
    ...
    def record_history(self):
        """Record the current positions of the player and ghost."""
        self.history.append({
    ...
    def review_history(self):
        """Display the paths of the player and the ghost."""
    ...
    def play(self):
        ...
        status_text = plt.text(0.5, 0.95, "", transform=plt.gca().transAxes, ha="center", fontsize=12, color="blue")
        ...
        def on_mouse_click(event):
            ...
            if closest_node in available_moves:
                ...
                # Additional sanity loss if ghost is hunting
                if self.ghost_hunt:
                    self.sanity -= 6  
                ...
                if self.player_position == self.ghost_position:
                    if not self.handle_ghost_encounter():
                        return  # End game on ghost defeat
        ...
        # Main game loop
        while self.sanity > 0:
            plt.pause(0.1)  # Allow UI updates and mouse click handling
...
if __name__ == "__main__":
    ...
    # Confirm if the player is ready to start
    ...
    if ready in ["yes", "y"]:
        ...
        while True:
            ...
            if mode == "1":
                ...
                game = Game(player_name)  # Player vs AI logic can be handled inside the Game class
        ...
        # Start the game and visualize the state
        game.display_user_stats()  # Display initial stats
        game.reset_stats()  # Reset stats for a fresh start
        game.play()  # Start the game with visualization
...
"""
    ...
    if not hasattr(game, 'G'):
        ...
        for u, v in edges:
            ...
            x2, y2 = game.pos[v]
            distance = ((x2 - x1)**2 + (y2 - y1)**2)**0.5
            game.G[u][v]['weight'] = distance  # Set the edge weight
